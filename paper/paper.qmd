...
```{r fig-col, fig.cap="Highest Average Predicted Listing Price in Seattle's 85 Acre Gated Community"}
testing_data |>
...
```{r fig-box, fig.cap='40 Year Old Homes Among the Lowest in Actual and Predicted Values'}
testing_data |>
  group_by(property_age) |>
  summarise(mean_price = mean(price) / 10^6,
            mean_predicted = mean(predicted_price) / 10^6) |>
  pivot_longer(names_to = "price_type", cols = c(mean_price, mean_predicted),
               values_to = "value") |>
  ...
  labs(x = "Property Age", y = "Average Price ($ millions)", color = "Price Type")
```
